...
- **Per-component outputs**: e.g. `dist/components/Dialog/Dialog.js` plus matching `dist/components/Dialog/Dialog.css` when styles exist (mirrors `src/components/...` folders).
...
- `react`
- `react-dom`
- `@radix-ui/react-dialog` (used by the sample `Dialog`)
...
- Published component entries (e.g. `dist/components/Dialog/Dialog.js`) begin with `"use client";` (for Next.js), then ESM imports, including a relative `import "./Dialog.css"` injected by `vite-plugin-lib-inject-css`.
...
- If the toolchain **does not** handle CSS imports, styles never enter the bundle; fix the build or import `./components/Dialog/Dialog.css` explicitly and ensure that path is processed.
...
The CSS for `Dialog` is pulled in with the JS module; you usually **do not** need an extra `import "...css"`.
...
| `./components/Dialog/Dialog` | Single-component entry |
| `./components/Dialog/Dialog.css` | Static CSS for that component (optional) |
...
2. Add an entry to `libEntry` in `vite.config.ts`, e.g. `"components/Dialog/Dialog": path.resolve(packageDir, "src/components/Dialog/Dialog.tsx")` (path mirrors `src/`).
